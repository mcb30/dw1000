...
/**
 * dw1000_rx_link_qual() - Check RX quality and calculate KPIs
 *
 * @dw:			DW1000 device
 */
static void dw1000_rx_link_qual(struct dw1000 *dw)
{
	...
	uint32_t finfo, power, noise, ampl1, ampl2, ampl3;
	uint32_t rxpacc, rxpsr, fpnrj, fppwr, overruns;
	unsigned int snr, fpr, psr;
	...
	/* Calculate a simplistic FP/Energy estimate */
	fpr = (fpnrj / power) >> 9;

	/* The reported LQI is upper limited */
	rx->lqi = (snr < DW1000_LQI_MAX) ? snr : DW1000_LQI_MAX;

	/* Timestamp is good by default */
	rx->timestamp_valid = true;
#if 0
	/* Check Preamble length */
	if (rxpacc < psr / 4) {
		rx->timestamp_valid = false;
		dev_warn_ratelimited(dw->dev, "STAMP SKIP: RXPACC %d < %d "
				     "SNR:%d FPR:%d PWR:%d NOISE:%d\n",
				     rxpacc, psr/4, snr, fpr, fppwr, noise);
	}
#endif
	/* Check S/N Ratio */
	if (snr < dw->snr_threshold) {
		rx->timestamp_valid = false;
		dev_warn_ratelimited(dw->dev, "STAMP SKIP: S/N:%d below "
				     "threshold %d; FPR:%d, PWR:%d, NOISE:%d\n",
				     snr, dw->snr_threshold, fpr, fppwr, noise);
	}

	/* Check F/P to Pwr Ratio */
	if (fpr < dw->fpr_threshold) {
		rx->timestamp_valid = false;
		dev_warn_ratelimited(dw->dev, "STAMP SKIP: F/P:%d below "
				     "threshold %d; SNR:%d, PWR:%d, NOISE:%d\n",
				     fpr, dw->fpr_threshold, snr, fppwr, noise);
	}

	/* Check background noise */
	if (noise > dw->noise_threshold) {
		rx->timestamp_valid = false;
		dev_warn_ratelimited(dw->dev, "STAMP SKIP: NOISE %d > %d "
				     "SNR:%d FPR:%d PWR:%d NOISE:%d\n",
				     noise, dw->noise_threshold, snr, fpr, fppwr, noise);
	}

	/* Check RX overruns */
	overruns = le16_to_cpu(rx->evc_ovr);
        if (unlikely(overruns != dw->rx_overruns)) {
		dev_warn_ratelimited(dw->dev, "STAMP SKIP: RX overrun(s) detected!\n");
...
static void dw1000_rx_irq(struct dw1000 *dw)
{
 ...
 err_recover:
	...
	if ((rc = regmap_write(dw->sys_ctrl.regs, DW1000_SYS_CTRL3,
		...
		dev_err(dw->dev, "RX recovery failed [#1]: %d\n", rc);
	}
	if ((rc = regmap_write(dw->sys_ctrl.regs, DW1000_SYS_CTRL3,
			DW1000_SYS_CTRL3_HRBPT)) != 0) {
		dev_err(dw->dev, "RX recovery failed [#2]: %d\n", rc);
	}
}
